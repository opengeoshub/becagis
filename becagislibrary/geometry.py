...
def skeleton(
    polygon: QgsGeometry, density, simplified_tol, postprocessing
) -> QgsFeature:
    ...
    if density > 0:
        ...
        points = processing.run(
            "qgis:pointsalonglines", parameters3, feedback=QgsProcessingFeedback()
        )
    elif density == 0:
        parameters3 = {"INPUT": poly_layer, "OUTPUT": "memory:points"}
        points = processing.run(
            "qgis:extractvertices", parameters3, feedback=QgsProcessingFeedback()
        )
    ...
    voronoipolygon = processing.run(
        "qgis:voronoipolygons", parameters4, feedback=QgsProcessingFeedback()
    )

    parameters5 = {
        "INPUT": voronoipolygon["OUTPUT"],
        "OUTPUT": "memory:voronoipolyline",
    }
    voronoipolyline = processing.run(
        "qgis:polygonstolines", parameters5, feedback=QgsProcessingFeedback()
    )

    parameters6 = {"INPUT": voronoipolyline["OUTPUT"], "OUTPUT": "memory:explode"}
    explode = processing.run(
        "qgis:explodelines", parameters6, feedback=QgsProcessingFeedback()
    )
    ...
    candidate = processing.run(
        "qgis:selectbylocation", parameters7, feedback=QgsProcessingFeedback()
    )

    parameters8 = {"INPUT": candidate["OUTPUT"], "OUTPUT": "memory:medialaxis"}
    medialaxis = processing.run(
        "qgis:saveselectedfeatures", parameters8, feedback=QgsProcessingFeedback()
    )

    parameters9 = {
        "INPUT": medialaxis["OUTPUT"],
        "OUTPUT": "memory:deleteduplicategeometries",
    }
    deleteduplicategeometries = processing.run(
        "qgis:deleteduplicategeometries", parameters9, feedback=QgsProcessingFeedback()
    )

    parameter10 = {
        "INPUT": deleteduplicategeometries["OUTPUT"],
        "OUTPUT": "memory:medialaxis_dissolve",
    }
    medialaxis_dissolve = processing.run(
        "qgis:dissolve", parameter10, feedback=QgsProcessingFeedback()
    )

    parameter11 = {
        "INPUT": medialaxis_dissolve["OUTPUT"],
        "METHOD": 0,
        "TOLERANCE": float(simplified_tol),
        "OUTPUT": "memory:simplify",
    }
    simplify = processing.run(
        "qgis:simplifygeometries", parameter11, feedback=QgsProcessingFeedback()
    )

    parameter12 = {"INPUT": simplify["OUTPUT"], "OUTPUT": "memory:explode"}
    explode = processing.run(
        "qgis:explodelines", parameter12, feedback=QgsProcessingFeedback()
    )
    ...
    ske = processing.run(
        "becagis:directionalmerge", parameter13, feedback=QgsProcessingFeedback()
    )
    ...
    if postprocessing:
        ...
        centerline_candidate = QgsVectorLayer(
            "Linestring", "Longest Geometry", "memory"
        )
        ...
        parameter14 = {
            "INPUT": centerline_candidate,
            "START_DISTANCE": float(max_length / 2),
            "END_DISTANCE": float(max_length / 2),
            "OUTPUT": "memory:extended",
        }
        extended = processing.run(
            "qgis:extendlines", parameter14, feedback=QgsProcessingFeedback()
        )

        parameter15 = {
            "INPUT": extended["OUTPUT"],
            "OVERLAY": poly_layer,
            "OVERLAY_FIELDS": None,
            "OUTPUT": "memory:intersected",
        }
        intersected = processing.run(
            "qgis:intersection", parameter15, feedback=QgsProcessingFeedback()
        )
...
def split_polygon(polygon: QgsGeometry, points_number: int, k: int):
    ...
    parameters = {
        "INPUT": clustered_layer,
        "BUFFER": 200,  # percentage
        "OUTPUT": "memory:voronoi",
    }
    voronoi_polygons = processing.run(
        "qgis:voronoipolygons", parameters, feedback=QgsProcessingFeedback()
    )
...
def k_means(points, k, max_iters=300, tol=1e-4):
    ...
    for _ in range(max_iters):
        ...
        new_centroids = np.array(
            [point_coords[labels == i].mean(axis=0) for i in range(k)]
        )
    ...
    result = [
        {"geometry": QgsPointXY(point[0], point[1]), "cluster_id": int(label)}
...
def meters_to_geographic_distance(distance_meters, src_crs, src_extent):
    ...
    N = a / np.sqrt(
        1 - e2 * (np.sin(lat_rad)) ** 2
    )  # Radius of curvature 1 degree vertical
    M = (
        a * (1 - e2) / (1 - e2 * (np.sin(lat_rad)) ** 2) ** (3 / 2.0)
...
def wedge_buffer(
    geometry, outer_radius, inner_radius=0, num_sectors=1, azimuth=0, num_segments=36
):
    """
    ...
    if num_sectors > 1:
        ...
        for sector in range(num_sectors):
            ...
            for i in range(num_segments + 1):
                angle = (
                    angle_start_rad
                    + i * (angle_end_rad - angle_start_rad) / num_segments
                )
            ...
            wedge_geometries.append(
                {"wedge_id": wedge_id_counter, "geometry": wedge_geom}
            )
        ...
        # Append dictionary with geometry and wedge_id
        wedge_geometries.append({"wedge_id": wedge_id_counter, "geometry": wedge_geom})

    return wedge_geometries


def spiralwedge_buffer(
    geometry,
    outer_radius,
    inner_radius=0,
    num_sectors=1,
    ...
    if num_sectors > 1:
        ...
        for sector in range(num_sectors):
            ...
            sector_outer_radius = outer_radius * (
                1 + increment_percentage / 100 * sector
            )
...
def mic(layer, tolerance):
    parameters1 = {"INPUT": layer, "TOLERANCE": tolerance, "OUTPUT": "memory:center"}
...
def longestline_insidepolygon():
    ...
    vl = QgsVectorLayer(
        "LineString?crs={}&index=yes".format(points.crs().authid()),
        "Longest_line",
        "memory",
    )
    pr = vl.dataProvider()

    polygons_idx = QgsSpatialIndex(
        polygons.getFeatures()
    )  # spatial index for polygonlayer

    with edit(vl):
        for point in points.getFeatures():  # iterate over pointlayer
            for pid in polygons_idx.intersects(
                point.geometry().boundingBox()
            ):  # iterate over intersecting polygons
                intersectingpolygon = polygons.getFeature(
                    pid
                )  # get current intersecting polygon
                ...
                tmpvl = QgsVectorLayer(
                    "MultiLineString?crs={}&index=yes".format(points.crs().authid()),
                    "tmplines",
                    "memory",
                )
                tmppr = tmpvl.dataProvider()
                for step in range(
                    180, 360, azimuthsteps
                ):  # iterate in the specified steps over the azimuths from current point
                    with edit(tmpvl):
                        tmplinefeat = QgsFeature()  # create a feature
                        stepopposite = step - 180  # reverse the azimuth
                        pointpoint = QgsPoint(
                            point.geometry().asPoint()
                        )  # make a QgsPoint out of the pointgeometry
                        startpoint = pointpoint.project(
                            linedist, step
                        )  # Project a point in the given azimuth (step) from the current point and declare it as startpoint of the line
                        endpoint = pointpoint.project(
                            linedist, stepopposite
                        )  # Project a point on the opposite site from the startpoint, so we get a straigt line
                        tmplinefeat.setGeometry(
                            intersectingpolygon.geometry().intersection(
                                QgsGeometry.fromPolyline([startpoint, endpoint])
                            )
                        )  # create a line inside the polygon
                ...
                blubb = processing.run(
                    "native:multiparttosingleparts",
                    {"INPUT": tmpvl, "OUTPUT": "TEMPORARY_OUTPUT"},
                )
                singlelines = blubb["OUTPUT"]
                singlelines_idx = QgsSpatialIndex(
                    singlelines.getFeatures()
                )  # spatial index for singlelines
                for slid in singlelines_idx.intersects(
                    point.geometry().boundingBox()
                ):  # iterate over features of singlelines that intersect with their point
                    slidf = singlelines.getFeature(
                        slid
                    )  # get the feature by the id from the index
                    linedict[slid] = (
                        slidf.geometry().length()
                    )  # add line's featureid and length to the temporary dictionary
                resultlines.append(
                    max(linedict, key=linedict.get, default=0)
                )  # append the longest line's featureid from the temporary dictionary to the resultlist
                resultfeatures = []  # create an empty list of matching features
                for slf in singlelines.getFeatures(
                    QgsFeatureRequest().setFilterFids(resultlines)
                ):  # iterate over the matching features in templayer
...
def lines_of_sight():
    ...
    vl = QgsVectorLayer(
        "LineString?crs={}&index=yes".format(pointlyr.crs().authid()),
        "myLayer",
        "memory",
    )
    ...
    visibility = QgsVectorLayer(
        "Polygon?crs={}&index=yes".format(vertices.crs().authid()), "myLayer", "memory"
    )
    points = []
    polygon_feature = QgsFeature()
    for (
        feature
    ) in vertices.getFeatures():  #    print (feature.id(),feature.geometry().asPoint())
