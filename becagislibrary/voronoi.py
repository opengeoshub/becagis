...
def skeleton(layer, field, density, tolerance):
    ...
    if density != 0:
        parameters3 = {"INPUT": layer, "DISTANCE": density, "OUTPUT": "memory:points"}
        points = processing.run("qgis:pointsalonglines", parameters3)
    else:
        parameters3 = {"INPUT": layer, "OUTPUT": "memory:points"}
        points = processing.run("qgis:extractvertices", parameters3)

    parameters4 = {
        "INPUT": points["OUTPUT"],
        "BUFFER": 0,
        "OUTPUT": "memory:voronoipolygon",
    }
    ...
    deleteduplicategeometries = processing.run(
        "qgis:deleteduplicategeometries", parameters9
    )
...
def isolation(layer, field):
    ...
    parameters1 = {"INPUT": layer, "OUTPUT": "memory:points"}
    delaunay_triangulation = processing.run("qgis:delaunaytriangulation", parameters1)

    parameters2 = {
        "INPUT": delaunay_triangulation["OUTPUT"],
        "OUTPUT": "memory:voronoipolyline",
    }
    delaunay_polyline = processing.run("qgis:polygonstolines", parameters2)

    parameters3 = {"INPUT": delaunay_polyline["OUTPUT"], "OUTPUT": "memory:explode"}
    ...
    deleteduplicategeometries = processing.run(
        "qgis:deleteduplicategeometries", parameters4
    )
    ...
    for feature in join_layer.getFeatures():
        join_layer.changeAttributeValue(
            feature.id(),
            idx,
            d.convertLengthMeasurement(
                d.measureLength(feature.geometry()), QgsUnitTypes.DistanceMeters
            ),
        )
...
def lec(layer, field):
    try:
        if layer.isValid():
            if layer.wkbType() == QgsWkbTypes.MultiPoint:
                parameters0 = {"INPUT": layer, "OUTPUT": "memory:singlepart"}
        ...
        temp = QgsVectorLayer(
            layer, QFileInfo(layer).baseName(), "ogr"
        )  # for running LEC in QGIS console
        if temp.wkbType() == QgsWkbTypes.MultiPoint:
            parameters0 = {"INPUT": temp, "OUTPUT": "memory:singlepart"}
            singlepart = processing.runAndLoadResults(
                "qgis:multiparttosingleparts", parameters0
            )
            point_layer = singlepart["OUTPUT"]
        else:
            point_layer = temp
    ...
    parameters1 = {"INPUT": point_layer, "BUFFER": 0, "OUTPUT": "memory:voronoipolygon"}
    ...
    parameter2_2 = {
        "INPUT": convexhull_vertices["OUTPUT"],
        "OUTPUT": "memory:convexhull_vertices_clean",
    }
    convexhull_vertices_clean = processing.run(
        "qgis:deleteduplicategeometries", parameter2_2
    )
    ...
    parameter4 = {"INPUT": voronoi_clip["OUTPUT"], "OUTPUT": "memory:voronoi_vertices"}
    voronoi_vertices = processing.run("qgis:extractvertices", parameter4)

    parameter5 = {
        "INPUT": voronoi_vertices["OUTPUT"],
        "OUTPUT": "memory:voronoi_vertices_clean",
    }
    voronoi_vertices_clean = processing.run(
        "qgis:deleteduplicategeometries", parameter5
    )

    parameter6 = {
        "INPUT": voronoi_vertices_clean["OUTPUT"],
    ...
    selection = centers.getFeatures(
        QgsFeatureRequest(QgsExpression('"HubDist"' + "=" + max_values_tr))
    )
    ids = [s.id() for s in selection]
    centers.selectByIds(ids)

    parameters8 = {"INPUT": centers, "OUTPUT": "memory:center"}
...
def closestfarthest(layer, field):
    try:
        ...
        temp = QgsVectorLayer(
            layer, QFileInfo(layer).baseName(), "ogr"
        )  # for running centerline in QGIS console
        if temp.wkbType() == QgsWkbTypes.MultiPoint:
            parameters0 = {"INPUT": temp, "OUTPUT": "memory:singlepart"}
            singlepart = processing.run("qgis:multiparttosingleparts", parameters0)
            point_layer = singlepart["OUTPUT"]
    ...
    ############
    parameters6 = {"INPUT": delaunay_polygon["OUTPUT"], "OUTPUT": "memory:convexhull"}
    ...
    parameters9 = {"INPUT": vertices_singlepart["OUTPUT"], "OUTPUT": "memory:clean"}
    clean = processing.run("qgis:deleteduplicategeometries", parameters9)
    vertices_clean = clean["OUTPUT"]

    parameters10 = {"INPUT": convexhull["OUTPUT"], "OUTPUT": "memory:convexhull_line"}
    line = processing.run("qgis:polygonstolines", parameters10)
    convexhull_line = line["OUTPUT"]

    parameters11 = {"INPUT": convexhull_line, "OUTPUT": "memory:explode"}
    ...
    parameters12 = {"INPUT": convexhull_explode, "OUTPUT": "memory:singlepart"}
    convexhull_singlepart = processing.run("qgis:multiparttosingleparts", parameters12)

    parameters13 = {
        "INPUT": convexhull_singlepart["OUTPUT"],
