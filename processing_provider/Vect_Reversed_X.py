...
class Reversed_X(QgsProcessingAlgorithm):
    ...
    def icon(self):
        return QIcon(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "images/vect_reversed_x.svg"
            )
        )
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(
                self.OUTPUT, self.tr("Reversed X Layer", "Reversed X Layer")
            )
        )
                    ...
                    if not geom.isMultipart():  # Single part
                        ...
                        for point in multipoint:
                            new_point = QgsPointXY(
                                self.Reversed_X_geom(point).asPoint()
                            )
