...
from becagis.becagislibrary.latlong import reversed_y
import processing
import os
from qgis.PyQt.QtGui import QIcon


class Reversed_Y(QgsProcessingAlgorithm):
    ...
    def displayName(self):
        return self.tr("Reversed Y", "Reversed Y")
    ...
    def icon(self):
        return QIcon(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "images/vect_reversed_y.svg"
            )
        )
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(
                self.OUTPUT, self.tr("Reversed Y Layer", "Reversed Y Layer")
            )
        )
            ...
            for current, feature in enumerate(features):
                ...
                if geom.type() == 0:  # Point
                    if not geom.isMultipart():  # Single part
                        new_geom = self.reversed_y_geom(feature.geometry().asPoint())
                        ...
                        for point in multipoint:
                            new_point = QgsPointXY(
                                self.reversed_y_geom(point).asPoint()
                            )
                    ...
                    if not geom.isMultipart():  # Single part
                        ...
                        for vertice in vertices:
                            new_vertice = self.reversed_y_geom(vertice)
                    ...
                    if not geom.isMultipart():  # Single part Polygon
                        ...
                        n = len(vertices[0])
                        for i in range(n):
                            new_vertice = self.reversed_y_geom(vertices[0][i])
                            new_vertices.append(QgsPointXY(new_vertice.asPoint()))
    ...
    def reversed_y_geom(self, geom):
        lat = geom.y()
        lon = geom.x()
        reversed_lat, reversed_lon = reversed_y(lat, lon)
