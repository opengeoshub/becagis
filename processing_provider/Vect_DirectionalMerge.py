...
class DirectionalMerge(QgsProcessingAlgorithm):
    ...
    def tags(self):
        return self.tr(
            "merge,dissolve,directional,touches,lines,connect,drainage,network"
        ).split(",")

    def icon(self):
        return QIcon(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "images/vect_mergeline.png"
            )
        )
    ...
    def shortHelpString(self):
        ...
        footer = (
            ...
            + """">
                      ...
                      <b>"""
            + self.tr(
                "Author: Leandro Franca - LFTools", "Tác giả: Leandro Franca - LFTools"
            )
            + """</b>                      
                    </div>"""
        )
    ...
    def initAlgorithm(self, config=None):
        ...
        tipo = [
            self.tr(
                "merge lines that have the same attributes",
                "Gộp các đường có cùng thuộc tính",
            ),
            self.tr(
                "keep the attributes of the longest line",
                "Lấy thuộc tính của đường dài nhất",
            ),
        ]
        ...
        self.addParameter(
            QgsProcessingParameterFeatureSink(
                self.OUTPUT, self.tr("Merged lines", "Đường gộp")
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        attributes = self.parameterAsEnum(parameters, self.TYPE, context)

        tol = self.parameterAsDouble(parameters, self.ANGLE, context)
        if tol is None or tol > 90 or tol < 0:
            raise QgsProcessingException(
                self.tr(
                    "The input angle must be between 0 and 90 degrees!",
                    "Góc phải >0 độ và <90 độ!",
                )
            )
                ...
                for i in range(0, length - 1):
                    ...
                    for j in range(i + 1, length):
                        ...
                        if att_A == att_B:
                            ...
                            if (P_end_A == P_ini_B) and (
                                fabs(ang_end_A - ang_ini_B) < tol
                                or fabs(360 - fabs(ang_end_A - ang_ini_B)) < tol
                            ):
                                merged = True
                                break
                            # 2 - Start point of A equals end point of B
                            elif (P_ini_A == P_end_B) and (
                                fabs(ang_ini_A - ang_end_B) < tol
                                or fabs(360 - fabs(ang_ini_A - ang_end_B)) < tol
                            ):
                                merged = True
                                break
                            # 3 - Start point of A equal to start point of B
                            elif (P_ini_A == P_ini_B) and (
                                fabs(ang_ini_A - self.contraAz(ang_ini_B)) < tol
                                or fabs(
                                    360 - fabs(ang_ini_A - self.contraAz(ang_ini_B))
                                )
                                < tol
                            ):
                                merged = True
                            ...
                            elif (P_end_A == P_end_B) and (
                                fabs(ang_end_A - self.contraAz(ang_end_B)) < tol
                                or fabs(
                                    360 - fabs(ang_end_A - self.contraAz(ang_end_B))
                                )
                                < tol
                            ):
                                merged = True
                ...
                for i in range(0, length - 1):
                    ...
                    for j in range(i + 1, length):
                        ...
                        # 3 - Start point of A equals end point of B
                            ...
                            fabs(ang_ini_A - self.contraAz(ang_ini_B)) < tol
                            or fabs(360 - fabs(ang_ini_A - self.contraAz(ang_ini_B)))
                            < tol
                        ):
                            merged = True
                            break
                        # 4 - End point of A equals end point of B
                            ...
                            fabs(ang_end_A - self.contraAz(ang_end_B)) < tol
                            or fabs(360 - fabs(ang_end_A - self.contraAz(ang_end_B)))
                            < tol
                        ):
                            merged = True
                            break
