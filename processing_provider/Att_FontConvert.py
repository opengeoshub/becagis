...
class FontConvert(QgsProcessingAlgorithm):
    ...
    def icon(self):
        return QIcon(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "images/att_fontconvert.png"
            )
        )
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            # QgsProcessingParameterFeatureSink(
            QgsProcessingParameterFeatureSink(self.OUTPUT, self.tr("Output", "Output"))
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        source_font_idx = self.parameterAsEnum(parameters, self.SOURCE_FONT, context)

        target_font_idx = self.parameterAsEnum(parameters, self.TARGET_FONT, context)

        case_idx = self.parameterAsEnum(parameters, self.CASE, context)
        ...
        for current, feat in enumerate(input_layer.getFeatures()):
            for field in fields:
                feat[field] = attribute.convertfont(
                    feat[field], source_font, target_font, case
                )
        ...
        feedback.pushInfo(
            self.tr("Font Convert completed successfully!", "Hoàn thành!")
        )
