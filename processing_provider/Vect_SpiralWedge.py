...
from qgis.core import QgsFeature, QgsField, QgsWkbTypes, QgsPropertyDefinition
...
from becagis.becagislibrary.geometry import (
    spiralwedge_buffer,
    meters_to_geographic_distance,
)
from becagis.becagislibrary.social_links import social_bw_html
...
class SpiralWedge(QgsProcessingFeatureBasedAlgorithm):
    ...
    def shortHelpString(self):
        social_BW = social_bw_html(os.path.dirname(os.path.dirname(__file__)))
        footer = (
            '''<div align="center">
                      <img src="'''
    ...
    def prepareAlgorithm(self, parameters, context, feedback):
        self.outer_radius = self.parameterAsDouble(
            parameters, self.OUTER_RADIUS, context
        )
        if self.outer_radius <= 0:
            feedback.reportError("Outer radius parameter must be greater than 0")
            return False
        self.outer_radius_dyn = QgsProcessingParameters.isDynamic(
            parameters, self.OUTER_RADIUS
        )
        if self.outer_radius_dyn:
            self.outer_radius_property = parameters[self.OUTER_RADIUS]

        self.inner_radius = self.parameterAsDouble(
            parameters, self.INNER_RADIUS, context
        )
        if self.outer_radius < 0:
            feedback.reportError(
                "Inner radius parameter must be equal to or greater than 0"
            )
            return False
        self.inner_radius_dyn = QgsProcessingParameters.isDynamic(
            parameters, self.INNER_RADIUS
        )
        ...
        if self.increment < 1:
            feedback.reportError(
                "Incerment percentage for outer radisu must be equal to or greater than 1 (%)"
            )
            return False
        self.increment_dyn = QgsProcessingParameters.isDynamic(
            parameters, self.INCREMENT
        )
        ...
        if self.src_crs.isGeographic():
            self.outer_radius = meters_to_geographic_distance(
                self.outer_radius, self.src_crs, self.src_extend
            )
            self.inner_radius = meters_to_geographic_distance(
                self.inner_radius, self.src_crs, self.src_extend
            )
    ...
    def processFeature(self, feature, context, feedback):
        ...
        try:
            ...
            if self.outer_radius_dyn:
                outer_rad, e = self.outer_radius_property.valueAsDouble(
                    context.expressionContext(), self.outer_radius
                )
                if self.src_crs.isGeographic():
                    outer_rad = meters_to_geographic_distance(
                        outer_rad, self.src_crs, self.src_extend
                    )
            ...
            if self.inner_radius_dyn:
                inner_rad, e = self.inner_radius_property.valueAsDouble(
                    context.expressionContext(), self.inner_radius
                )
                if self.src_crs.isGeographic():
                    inner_rad = meters_to_geographic_distance(
                        inner_rad, self.src_crs, self.src_extend
                    )
            ...
            if self.increment_dyn:
                increment_percentage, e = self.sectnum_property.valueAsInt(
                    context.expressionContext(), self.increment
                )
            ...
            if self.sectnum_dyn:
                sect_num, e = self.sectnum_property.valueAsInt(
                    context.expressionContext(), self.sectnum
                )
            ...
            if self.azimuth_dyn:
                azimuth_degree, e = self.azimuth_property.valueAsDouble(
            ...
            wedge_geoms = spiralwedge_buffer(
                geom,
                outer_rad,
                inner_rad,
                sect_num,
