...
class Wedge(QgsProcessingFeatureBasedAlgorithm):
    ...
    def icon(self):
        return QIcon(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "images/vect_wedge.png"
            )
        )
    ...
    def prepareAlgorithm(self, parameters, context, feedback):
        ...
        if self.src_crs.isGeographic():
            self.outer_radius = meters_to_geographic_distance(
                self.outer_radius, self.src_crs, self.src_extent
            )
            self.inner_radius = meters_to_geographic_distance(
                self.inner_radius, self.src_crs, self.src_extent
            )
    ...
    def processFeature(self, feature, context, feedback):
        ...
        try:
            ...
            if self.outer_radius_dyn:
                ...
                if self.src_crs.isGeographic():
                    outer_rad = meters_to_geographic_distance(
                        outer_rad, self.src_crs, self.src_extent
                    )
            ...
            if self.inner_radius_dyn:
                ...
                if self.src_crs.isGeographic():
                    inner_rad = meters_to_geographic_distance(
                        inner_rad, self.src_crs, self.src_extent
                    )
            ...
            wedge_geoms = wedge_buffer(
                geom, outer_rad, inner_rad, sect_num, azimuth_degree, seg_num
            )
