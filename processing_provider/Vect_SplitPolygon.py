...
from qgis.core import QgsFeature, QgsPropertyDefinition
...
class SplitPolygon(QgsProcessingFeatureBasedAlgorithm):
    ...
    def prepareAlgorithm(self, parameters, context, feedback):
        ...
        self.random_points = self.parameterAsInt(
            parameters, self.RANDOM_POINTS, context
        )
        if self.random_points < 10:
            feedback.reportError("Outer radius parameter must be at least 10")
            return False
        self.random_points_dyn = QgsProcessingParameters.isDynamic(
            parameters, self.RANDOM_POINTS
        )
    ...
    def processFeature(self, feature, context, feedback):
        ...
        if self.random_points_dyn:
            random_points_no, e = self.random_points_property.valueAsInt(
                context.expressionContext(), self.random_points
            )
        ...
        # Evaluate parts
        if self.parts_dyn:
            parts_no, e = self.parts_property.valueAsInt(
                context.expressionContext(), self.parts
            )
