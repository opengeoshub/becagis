...
from qgis.core import QgsFeature, QgsWkbTypes, QgsPropertyDefinition
...
class Skeleton(QgsProcessingFeatureBasedAlgorithm):
    ...
    def icon(self):
        return QIcon(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "images/vect_skeleton.svg"
            )
        )
    ...
    def initParameters(self, config=None):
        ...
        param.setDynamicPropertyDefinition(
            QgsPropertyDefinition(
                self.DENSITY, self.tr("Density (meters)"), QgsPropertyDefinition.Double
            )
        )
    ...
    def prepareAlgorithm(self, parameters, context, feedback):
        ...
        self.postprocessing = self.parameterAsBoolean(
            parameters, self.POSTPROCESSING, context
        )
        ...
        if self.src_crs.isGeographic():
            self.density = meters_to_geographic_distance(
                self.density, self.src_crs, self.src_extent
            )
    ...
    def processFeature(self, feature, context, feedback):
        ...
        try:
            # Evaluate density
            if self.density_dyn:
                density_dist, e = self.density_dyn_property.valueAsDouble(
                    context.expressionContext(), self.density
                )
                if self.src_crs.isGeographic():
                    density_dist = meters_to_geographic_distance(
                        density_dist, self.src_crs, self.src_extent
                    )
