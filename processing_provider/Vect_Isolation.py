...
class Isolation(QgsProcessingAlgorithm):
    ...
    def icon(self):
        return QIcon(
            os.path.join(
                os.path.dirname(os.path.dirname(__file__)), "images/vect_isolation.png"
            )
        )
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(
            ...
            QgsProcessingParameterFeatureSink(
                self.OUTPUT, self.tr("Isolated Point", "Isolated Point")
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if unique_field is None:
            raise QgsProcessingException(
                self.invalidSourceError(parameters, self.UNIQUE_FIELD)
            )

        # result = isolation(parameters[self.INPUT], unique_field)

        isolated_field, max_distance = isolation(
            parameters[self.INPUT], parameters[self.UNIQUE_FIELD]
        )
        ...
        if source.crs().isGeographic():
            ...
            max_distance = format(
                np.degrees(theta_max_distance), "f"
            )  # Radian to degree
        ...
        return {self.OUTPUT: isolated["OUTPUT"], self.CIRCLE: circle["OUTPUT"]}
